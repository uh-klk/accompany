...
void MainWindow::initialiseGUI()
{
       ...
       qry = "SELECT * FROM Sequences where scenario = 'user' and name not like 'reset%' AND experimentalLocationId = " +  expLocation;
...
void MainWindow::on_alreadyKnowWidget_currentChanged(int index)
{
       qDebug()<<"tab"<<index;
        ...
        switch (index)
        {
            ...
            case 0:
              query.prepare("SELECT * FROM Sequences where (scenario = 'user' or scenario = 'High Level') and name not like 'reset%' \
